...
//import com.github.mikephil.charting.data.VolumeDataSet;
 ...
 */
public class VolumeDataExtract extends DataExtract<BarData, BarEntry> {
    @Override
    BarData createData() {
        return new BarData();
    }
    ...
    BarEntry createEntry(ReadableArray values, int index) {
        ...
        if (ReadableType.Map.equals(values.getType(index))) {
            ...
            if (ReadableType.Array.equals(map.getType("y"))) {
                ...
                VolumeColorData vd=null;
                if(ReadableType.Number.equals(map.getType("colorIndex")))
                    vd = new VolumeColorData(map.getInt("colorIndex"));
                entry = new BarEntry(x, (float) map.getDouble("y"), vd);
            } else {
    ...
    void dataSetConfig(Chart chart, IDataSet<BarEntry> dataSet, ReadableMap config) {
        VolumeDataSet barDataSet = (VolumeDataSet) dataSet;
